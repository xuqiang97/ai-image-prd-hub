...
现有生成成图为 JPG，任务由创建人查看、审核和编辑。美工商品图、美工 A+、运营 A+ 全部图片审完后返回审核工作台并弹出推送二次确认，确认后自动推送，取消则留在审核工作台，可再次进入沉浸式大屏审图继续二改；运营非 A+ 由 Listing 按现有规则查询审核通过的可用图片，任务完成后禁止二改。人物识别、必要元数据打标及“阿里大模型 OCR → 公司内部文本违禁检测”已有能力，本期复用并接入二改流程。

## 需求概述

美工版、运营版从 Tab3「待审核」进入审核工作台，再进入沉浸式大屏审图；在大屏审图底部增加“编辑当前图片”，支持在线编辑和本地 JPG/PNG 上传。PNG 按原尺寸转白底 JPG；两种方式均由用户预览确认后统一检测、保存，更新原任务当前图片并重新审核。
...
flowchart LR
    subgraph Edit["编辑与提交"]
        direction TB
        A["大屏审图：打开编辑器"] --> B["编辑或上传，预览并确认替换"]
        B --> C["新增文案校验（如有）"]
        C -->|通过或无需检查| D["最终 JPG 校验，提交替换"]
    end

    subgraph Detect["检测与保存"]
        direction TB
        E["人物检测"] --> F["按人物结果增删、去重 XMP 标记并验证"]
        F --> G["整图 OCR 提取文字，并进行文本检测"]
        G -->|通过或明确无文字| H["最终文件复检，保存替换结果"]
    end

    subgraph Review["回到审核"]
        direction TB
        I["返回大屏审图，更新图片及两项检测结果"] --> J["当前图片重新审核，工作台同步"]
        J --> K["按原业务规则推送或供 Listing 查询"]
    end
...
二改在第 1 节沉浸式大屏审图入口内进行，沿用现有审核流程及以下截止边界：

| 业务链路 | 审核流程及编辑截止点 |
|---|---|
| 美工商品图、美工 A+、运营 A+ | 通过沉浸式大屏审图进入编辑器二改。全部图片审完后返回审核工作台，弹出“确认推送？”；确认后自动推送并禁止二改，取消则留在审核工作台，可再次进入沉浸式大屏审图打开编辑器。推送确认弹窗期间不能编辑。 |
...
- ERP 接入须落实替换请求、结果查询及审核刷新的等待上限和过期回调隔离；达到上限按第 3 节恢复，不能因宿主调用未返回而一直停留在处理中。具体时限与实现方式由研发结合真实接口确定。阶段回报及前端阶段顺序校验须适配第 3 节人物检测后先处理 XMP、再进行 OCR 的顺序，避免将有效 OCR 回报误判为倒退；保留现有四段进度展示。
...
| 7. 审核回流与推送 | 替换后返回大屏审图原图片位置，图片及两项检测结果按第 4 节同步，OCR 无文字文案正确，筛选变化不移走当前图；工作台卡片、审核状态及顶部数量同步，仅该图重新待审核。覆盖“全部审完 → 取消推送 → 再进大屏二改 → 返回工作台”，其他图片结果保留，再次进入、工作台直接审核及推送条件均使用新版本。旧图审核被拒绝，加载最新图后重新审核。三条主动推送链路审完返回工作台并弹出推送确认，确认后自动推送，取消留在工作台，弹窗期间不能编辑；原有推送失败处理保持可用，不开放二改。替换与推送确认、运营非 A+ 任务完成的两种生效顺序符合第 5 节。 |
