...
本文用于说明店铺、ASIN、Listing、五点描述、尺寸和参考图之间的数据依赖与最终有效口径。M-37095 本期正式改造范围仅为多套资料筛选、排序、最终命中对象字段一致性和 MSKU/SKU 成对取值；本文记录的其他页面、任务保存、状态判断和人工完善规则均为现状基线或研发技术评估项，不属于 M-37095 新增开发范围。未来按图片类型和位置配置参考图的策略不在本文范围内。
...
flowchart TD
    ...
    B --> E["sourceId + ASIN 查询 Amazon Listing取数接口"]
    D --> E
    ...
    L -->|"是"| M["允许提交生图"]
    L -->|"否"| N["业务手动上传至少1张参考图并完善商品文本信息和生图模型配置项"]
    N --> M
```
...
以下为运营版现有字段数据源分工，用于说明完整取数链路，不属于 M-37095 新增开发范围。运营版按字段的数据能力固定划分数据源，不对同一个字段在 Listing 与产品库之间执行自动降级、补齐或覆盖。
...
以下为单个新增和编辑抽屉的现状保存规则，不属于 M-37095 新增开发范围。单个新增任务查询只负责自动回填接口可取得的数据。新增任务抽屉和编辑任务抽屉在保存时，商品文本信息、参考图和生图模型配置项均按现有必填规则校验：
...
本节仅记录研发技术评估方向，不构成 M-37095 新增的强制改造范围、技术方案或性能验收指标。本链路为实时查询，涉及店铺上下文、Listing、五点描述、尺寸和图片多个数据源；页面继续通过一次查询取得聚合结果，后端可结合现有实现评估扩展 `listingV2ToAiImage` 或新增聚合接口，不要求前端直接串行调用多个内部接口。
...
| Amazon Listing 取数接口 | http://showdoc.zhcxkj.com/web/#/88/5259 | 查询 Listing 并按 M-37095 择优 |
| Listing 大字段属性接口 | http://showdoc.zhcxkj.com/web/#/88/5569 | 按 `sourceId + msku` 查询 `bulletPoint` |
